<div x-data class="ld-filter-control flex flex-col">
    <div class="w-full relative flex">
        ...
        @if(filled($this->activeDateFilters[$index]['start'] ?? null))
    ...
    <div class="w-full relative flex items-center">
        <input x-ref="end" class="w-full pr-8 m-1 text-sm leading-4 block rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" type="date"
            wire:change="doDateFilterEnd('{{ $index }}', $event.target.value)" value="{{$this->activeDateFilters[$index]['end'] ?? null}}" style="padding-bottom: 5px" />
        @if(filled($this->activeDateFilters[$index]['end'] ?? null))
        <div class="absolute inset-y-0 right-0 pr-2 flex items-center">
